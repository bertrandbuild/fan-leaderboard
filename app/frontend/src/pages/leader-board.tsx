...
export function LeaderBoard() {
  ...
  const filteredPools = useMemo(() => {
    let filteredPools =
      activeStatus === "all"
        ? pools
        : pools.filter((pool) => pool.status === activeStatus);
    ...
    <div className="space-y-6 w-full px-4 sm:px-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 w-full mb-6">
        <div className="flex gap-2 flex-wrap">
          ...
          <Button
            onClick={() => setActiveStatus("Ending Soon")}
            className={
              activeStatus === "Ending Soon"
                ? "bg-orange-500 hover:bg-orange-600 text-white"
                : "bg-slate-700 hover:bg-slate-600 text-slate-300"
            }
          >
          ...
          {topPools.length === 1 ? (
            ...
            <Card className="w-full max-w-xs bg-slate-800 border-2 border-emerald-400 shadow-lg">
              <CardContent className="flex flex-col items-center p-4 md:p-6">
                <div className="w-12 md:w-16 h-12 md:h-16 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold text-xl md:text-2xl mb-2">
                  1
                </div>
                <div className="text-white font-bold text-lg md:text-xl text-center">
                  {topPools[0].name}
                </div>
                <div className="text-emerald-300 text-sm md:text-base font-semibold">
                  ${(topPools[0].totalLiquidity / 1000).toFixed(0)}K
                </div>
                <Badge
                  className={`mt-2 ${
                    topPools[0].status === "Active"
                      ? "bg-green-500"
                      : topPools[0].status === "Ending Soon"
                      ? "bg-orange-500"
                      : "bg-slate-500"
                  } text-white`}
                >
              ...
              <Card className="flex-1 max-w-xs bg-slate-800 border-slate-700">
                <CardContent className="flex flex-col items-center p-4 md:p-6">
                  ...
                  <div className="text-slate-400 text-xs md:text-sm">
                    ${(topPools[1].totalLiquidity / 1000).toFixed(0)}K
                  </div>
                  <Badge
                    className={`mt-2 ${
                      topPools[1].status === "Active"
                        ? "bg-green-500"
                        : topPools[1].status === "Ending Soon"
                        ? "bg-orange-500"
                        : "bg-slate-500"
                    } text-white`}
                  >
            ...
            <>
              ...
              <Card className="flex-1 max-w-xs bg-slate-800 border-slate-700 scale-90 z-10">
                <CardContent className="flex flex-col items-center p-2 md:p-4">
                  ...
                  <div className="text-slate-400 text-xs md:text-sm">
                    ${(topPools[2].totalLiquidity / 1000).toFixed(0)}K
                  </div>
                  <Badge
                    className={`mt-2 ${
                      topPools[2].status === "Active"
                        ? "bg-green-500"
                        : topPools[2].status === "Ending Soon"
                        ? "bg-orange-500"
                        : "bg-slate-500"
                    } text-white`}
                  >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                >
                  <TabsList className="bg-slate-700 border-slate-600 grid w-full grid-cols-3 sm:w-auto">
                    <TabsTrigger value="top" className="text-sm py-2 px-4">
                      Top Yappers
                    </TabsTrigger>
                    <TabsTrigger value="emerging" className="text-sm py-2 px-4">
                      Emerging
                    </TabsTrigger>
                  </TabsList>
                </Tabs>
                    ...
                    <div className="hidden md:block">
                      ...
                      <TableHeader>
                        <TableRow className="border-slate-700">
                          ...
                          <TableHead className="text-slate-400 min-w-[280px] text-base">
                            Name
                          </TableHead>
                          <TableHead className="text-slate-400 text-center w-32 text-base">
                            Total Yaps
                          </TableHead>
                          <TableHead className="text-slate-400 text-center w-32 text-base">
                            Earned Yaps
                          </TableHead>
                          <TableHead className="text-slate-400 text-center w-40 text-base">
                            Smart Followers
                          </TableHead>
                          <TableHead className="text-slate-400 text-center w-32 text-base">
                            Followers
                          </TableHead>
                          <TableHead className="text-slate-400 text-center w-28 text-base">
                            Smart %
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                              ...
                              <div className="flex items-center gap-4">
                                <Avatar className="w-12 h-12">
                                  <AvatarFallback className="bg-cyan-500 text-white font-medium text-sm">
                                    {yapper.avatar ||
                                      yapper.name.slice(0, 2).toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                                <div>
