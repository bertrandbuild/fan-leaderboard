...
export function SocialManager() {
  ...
  function handleSelectCeleb(celeb: Celebrity) {
    ...
    const fresh = celebrities.find(c => c.socialNetworkId === celeb.socialNetworkId)
    if (fresh) {
      setSelectedCeleb(fresh)
      setEditWeight(String(fresh.weight))
    }
  }
  function handleSaveWeight() {
    if (selectedCeleb) {
      setCelebrities((prev) =>
        prev.map((c) =>
          c.socialNetworkId === selectedCeleb.socialNetworkId
            ? { ...c, weight: Number(editWeight) }
            : c
        )
      )
      setSelectedCeleb(null)
      setEditWeight("")
      toast({ title: "Modification saved", description: `${selectedCeleb.name} updated!` })
    }
  }

  // Ajout de célébrité
  function handleAddCeleb(e: React.FormEvent) {
    e.preventDefault()
    if (!newCeleb.name || !newCeleb.socialNetworkId || !newCeleb.team) return
    setCelebrities(prev => [
      { ...newCeleb, weight: Number(newCeleb.weight) || 10 },
      ...prev,
    ])
    setNewCeleb({ name: "", socialNetworkId: "", team: "", weight: 10 })
    setShowAddForm(false)
    toast({ title: "Celebrity added", description: `${newCeleb.name} added!` })
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-slate-800 border-slate-700 flex flex-col">
        ...
        <CardContent className="space-y-3">
          {/* Add Form */}
          {showAddForm && (
            <form onSubmit={handleAddCeleb} className="bg-slate-700 rounded-lg p-4 mb-2 space-y-2">
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Input
                  ...
                  value={newCeleb.socialNetworkId}
                  onChange={e => setNewCeleb(v => ({ ...v, socialNetworkId: e.target.value }))}
                  className="bg-slate-800 border-slate-600 text-white"
                />
                <Input
                  placeholder="Team"
                  value={newCeleb.team}
                  onChange={e => setNewCeleb(v => ({ ...v, team: e.target.value }))}
                  className="bg-slate-800 border-slate-600 text-white"
                />
                <Input
                  type="number"
                  ...
                  onChange={e => setNewCeleb(v => ({ ...v, weight: Number(e.target.value) }))}
                  className="bg-slate-800 border-slate-600 text-white w-20"
                />
              </div>
              <div className="flex gap-2 mt-2">
                <Button type="submit" className="bg-yellow-500 hover:bg-yellow-600 text-white">Add</Button>
                <Button
                  type="button"
                  variant="outline"
